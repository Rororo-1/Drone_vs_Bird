...
На графіках видно, що модель за 30 епох ще не досягла збіжності. Однак, вона вже має низькі втрати на детекції і класифікації, добре знаходить об'єкти: recall~85 з точністю ~ 95.
Також, основниі параметри детекції "mAP" теж вже досягли високих значень

===========================================================================

![Labels](images/labels.jpg)
Тут ми можемо побачити невеликий дисбаланс в класах, але для детекції це зазвичай не проблема. Також, тут видно, що в датасеті об'єкти мають маленькі бокси і здебільшого розташоані по центру кадру.
...
З цієї гіфки добре видно, що модель починає плутатись і детектити дрон, як пташку, коли в кадр потрапляють гілки. Я думаю, це через те, що в даті багато зображень де птахи на гілках. Аби це виправити, можна видалити всі зображення з птахами на гілках, і доповнити датасет синтетичними данними.
...
## Плани на майбутнє:
1) Присвоювати додатковий клас кожному задетекченому об'єкту, аби розуміти, що з кадру в кадр ми відстежуємо один об'єкт.
2) Додати пеердбачення розташування боксу на насткпному кадрі, задля прогнозування руху і покращенню супроводження об'єкта між кадрами
3) Додати модель розпізнавання аномалій, що дозволить розрізняти нечіткі/маленькі об'єкти. Адже на прикладі, добре видно, що модель не бачить дрон-перехоплювач, що летить за шахедом
